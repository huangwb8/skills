...
4. **更新用户文档（强制）**：
   ...
   - 如优化涉及模型相关变更，使用 which-model skill 更新 WHICHMODEL 章节
...
`description` 必须是**单行格式**，不使用 YAML 多行语法（`|` 或 `>`）。所有内容应合并为一段连贯的文字：

```yaml
# ✅ 正确：单行格式
description: 当用户明确要求"测试技能"时使用。通过多轮批判性测试系统化发现问题并沉淀文档。⚠️ 不适用：用户只是想优化功能（应直接修改）。
...
**合并策略**：
- 核心功能说明 + 关键能力 → 用句号连接，合并为一段
- 负向约束（不适用场景）→ 使用 `⚠️` 内嵌到同一段落末尾
- 列表内容 → 转换为简短短语，用分号或顿号连接
...
**实践要点**：
- 避免在 SKILL.md 中添加"版本历史"章节
- 避免用多层引用分散内容（应保持扁平化或集中到 references/）
- 避免添加宣传性标记（如"新增"、"v2.x.0 新增"）
- 推荐当 SKILL.md 接近 500 行时，主动拆分详细内容到 references/
- 推荐使用 Progressive Disclosure 策略——核心逻辑在 SKILL.md，细节在 references/
...
**实践要点**：
- 禁止在 SKILL.md 的 YAML frontmatter 中添加 `metadata.version` 字段
- 禁止在 SKILL.md 正文中直接写明"当前版本：v1.0.0"
- 禁止在 README.md 中硬编码版本号（如"最新版本 v1.0.0"）
- 推荐在 README.md 中写明"版本信息见 config.yaml:skill_info.version"
- 推荐使用脚本或构建工具动态读取 config.yaml 中的版本号（如需要）
