...
- `bensz-collect-bugs` 是一个 Agent Skill，仅用于记录 Bensz Agent Skill 或 Bensz 基础设施本身的设计缺陷（最小复现、影响范围、workaround），不得用于记录用户项目业务问题，也不得污染用户项目交付物。
...
- SKILL.md 中必须包含 `bensz-collect-bugs` 约束章节，并明确它是一个用于缺陷留痕与按需上报的 Agent Skill：
  - **适用范围**：仅处理 Bensz Agent Skill 或 Bensz 基础设施本身的设计缺陷（流程漏判、输入契约不完整、环境假设错误等），排除用户数据有误、第三方服务抖动、用户主动改源码、模型偶发波动
...
`SKILL.md` 的 `description` 与正文必须严格分工：`description` 只负责说明 Skill 的核心用途和触发条件，用于判断“什么时候应该调用”；正文负责完整执行契约，包括输入、步骤、输出、产物管理、校验、失败恢复与约束。除非某项信息确实影响触发判断，不得在 `description` 中枚举具体输出文件、目录结构、命令参数、内部实现、详细规则或易变默认值；这些内容应放在正文对应章节，详细参数以 `config.yaml` 为单一真相来源。修改 Skill 时必须检查 description 与正文是否重复、是否存在触发边界遗漏，以及正文是否承接了所有执行细节。脚本必须基于 `Path(__file__).resolve()` 定位自身资产，不能依赖用户当前工作目录。
