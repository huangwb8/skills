...
本节对齐 `/Volumes/2T01/Github/sub2api/docs/prompts/005-bensz-skill-workspace.md`，为需要落盘的 AI 任务提供统一、可审查且不污染项目交付物的中间文件管理。来源文档的目录名是 `.bensz-api`（本文统一采用该拼写；用户口语中的 `.benszapi` 指同一机制）。它不替代具体业务 Skill 的专业流程。
