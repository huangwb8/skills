...
- `install-bensz-skills` 新增由安装器独占管理的 `~/.bensz-skills/envs/benszapi` Conda 运行时，统一维护最新生产版 BSK，并生成固定 `~/.bensz-skills/bin/bsk` 入口；本地完整安装器与 Python 3.8+ bootstrap 新增 ensure/status/force-update 入口。
...
- BSK 运行声明验证允许新 Skill 省略 Kernel 版本；旧版本和 capability 字段仍保持只读兼容。
...
- `bensz-skill-kernel` 更新至 2.1.3（版本号由用户确认）；运行快照改为不可覆盖且读取时校验 payload/hash/ID，Verifier 绑定覆盖真实 Markdown 契约、组件计划与 helper 资产，幂等 transition 保持身份模式字段一致。184 个包内测试与 218 个仓库测试通过后，sdist/wheel 通过 `twine check` 并发布至 PyPI，托管运行时已安装并验证 `bsk --version`、`capabilities` 与 `diagnostics`。
- 静默更新在 72 小时 TTL 到期后同步维护托管运行时，避免 PATH、项目 Python 与系统 Python 加载不同 Kernel。
...
- 发布 `bensz-skill-kernel` 1.0.3 到 PyPI：包版本随仓库 v5.0.4 发布同步提升，122 个包内测试全部通过；本轮 Kernel 源码无变更，根级与 Kernel 中英 README 经 `write-readme` 流程与 `check_readme_pair.py` 核验已与源代码对齐，无需改写。
