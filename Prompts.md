...
基于 /Volumes/2T01/Github/sub2api/docs/plans/2026-09-06-install-bensz-skills-bootstrap-hardening-plan.md 优化skill。跳过计划与根因核验步骤， 直接开始落实计划。 计划里的所有阶段的问题（p0-p2级）都要解决。如果工作时有疑问，或者有更好的方案，自己选个最优方案优化，不要问我。不要破坏其它已经存在的功能。要保证最终成品能正常、稳定、高效地工作。
...
- 首先，用户肯定有一些需求； ai要充分理解用户的需求。通常，用户还会提供原始输入（比如，一个分析所依赖的数据）
- 这时，ai应该开始规划
  - 我已经知道原始数据是什么。为了完成需求，我需要做哪些分析？
  - 分析需要哪些数据？一般来说， 基于原始数据和一些初步会析， 肯定会有一些中间数据。 有一些中间数据是有必要缓存的，因为它有利于用户反复进行调试。要怎么适当地设计数据的分步骤缓存、断点续算？
  - OK，把上述问题都想清楚，这个时候就可以开始落实分析代码。先一下子把代码写出来，然后开独立的子agent至少3次串行检查一下代码有没有问题。有问题就改，没问题就跳过。 
  - 这个时候就开始写代码、跑测试。相关的约定基本同现在 skills/beta/bensz-rmd-rules 
- 我还希望ai的输出有一些约定的目录要求。 我个人的习惯是：假设项目根目录是 .  ，那么：
  - `./products/` 可以用来保存值得用户审查的、可能被其它流程复用的中间文件
  - `./raw` 一般是用户保存原始文件和材料的地方
  - `./reports` 一般是一些很重要的输出，包括图、表之类的； 用户通常是用它们来写论文的。它们往往也是在Rmd/HTml里展示用的东西。
  - 当然， 其实一般来说是有 .bensz-api/ 这个中间目录。 但这里适合放一下不太重要的、没必要给用户看的但为了维持流程运转而不得不承担的中间文件、脚本等
  - 其它的 rmd html 输出的规范，基本同现在

基于上述的描述，结合目前  skills/beta/bensz-rmd-rules 的默认约定， 请你给一个经典分析的 step-by-step 细节，让我看看你的想法对不对。 

---

skills/beta/bensz-rmd-rules 目前对于分析过程中产生的中间数据是如何处理的？有时候，一个分析可能由很多步骤组成，其中一些步骤甚至是非常耗时的。 因此， 很有必要把这些步骤的关键数据缓存下来； 否则，当用户调试的时候，就会陷入无尽的从头长期分析当中。 正确，但不优雅，也很笨。真实的人类分析，往往会将关键的中间数据保存好。一般来说，这个数据有这些特点：

- 需要经过长期、大量的运算才可以获得
- 数据结构应该是人类很容易审查的格式
- 以固定的规范来保存

目前，该skill在这方面做好了什么?还有什么没做好？
