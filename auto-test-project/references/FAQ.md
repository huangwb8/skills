...
1. 重新阅读 `plans/vYYYYMMDDHHMM.md` 的问题清单（确认每个问题都有编号，如 `P0-1`）。
2. 检查 `tests/vYYYYMMDDHHMM/TEST_REPORT.md` 是否包含相同编号的修复记录。
...
- 脚本会在 `--project-root` 下创建 `plans/` 与 `tests/`；默认拒绝将系统根目录或用户主目录作为 project-root，如需覆盖请显式使用 `--allow-unsafe-root`。
- 中间产物统一放进 `tests/<session>/_artifacts/` 与 `tests/<session>/_scripts/`，避免在项目根散落临时文件。
