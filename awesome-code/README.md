...
这个 skill 是复杂开发任务的协调器：它会先分析任务，再推荐合适的专业代理，再决定用单任务、顺序还是并行策略推进；如果只是一个很小的改动，通常不需要动用它。
...
- 推荐代理列表，以及每个代理负责的方向。
- 协调策略说明：单任务、顺序执行或并行推进。
- 面向落地执行的下一步，而不只是抽象建议。
- 如使用脚本入口，还会得到可执行的路径与命令。
