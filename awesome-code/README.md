...
- 需要实施计划时，优先产出“问题、影响、目标、改进方向、验收”的白话主线；仅在能澄清接口、风险或验证方式时才补充技术细节，避免把计划写成机械化操作清单。
...
### Q：实施计划会包含逐行代码、测试步骤和提交命令吗？

A：默认不会。计划先帮助读者理解问题、目标和取舍；只有这些细节是准确实施、验证或降低风险所必需时，才会作为简短的技术补充出现。正式计划保存到 `docs/plans/`，任务过程材料保留在 `.bensz-api/` 工作区。
