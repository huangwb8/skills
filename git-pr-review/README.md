...
输出：项目根目录下 1 份 Markdown 审查报告；所有中间文件放到 `.git-pr-review/`
```
...
输出：项目根目录下 1 份最终 Markdown 审查报告；`.git-pr-review/` 里保留并行独立评审的全部中间产物
```
...
- 会把所有中间文件放进工作目录下的 `.git-pr-review/`
- 会在项目根目录生成最终报告
...
- `Git-PR-Review_{repo}_{pr}_{timestamp}.md`
...
- `.git-pr-review/run_<timestamp>_<repo>_<pr>/`
...
- `parallel_review/parallel_runs/.parallel_vibe/<project_id>/...`：独立 reviewer threads 产物
...
可以。默认目录是 `.git-pr-review/`，但如果你明确指定其他目录，skill 会按你的目录来。
