...
bsk verifier run markdown.references --version 1.0.0 --input README.md
```

当前内置示例：

- `artifact.file-exists`：通用文件存在性检查，标签 `common`、`filesystem`、`deterministic`。
- `markdown.references`：Markdown 引用、站内 anchor 和 HTTP(S) 可达性检查；版本通过 `--version` 指定，当前为 `1.0.0`，标签 `vertical`、`markdown`、`references`、`network-read`。
