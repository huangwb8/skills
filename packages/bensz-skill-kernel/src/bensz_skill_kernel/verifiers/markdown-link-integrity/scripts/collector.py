"""Markdown-specific fact collection for the link-integrity verifier.

This module deliberately stays next to the directory verifier.  The Kernel
owns verifier discovery, execution, result normalisation and event handling;
Markdown parsing and link probing belong to this verifier's domain boundary.
"""
...
def _skip_reason(url: str, blacklist: tuple[str, ...], whitelist: tuple[str, ...]) -> str | None:
    ...
    if whitelist and not any(
        hostname == item.lower().lstrip('*.')
        or hostname.endswith('.' + item.lower().lstrip('*.'))
