...
MESSAGES_EN = Messages(
    ...
    error_source_root_not_found="Failed to auto-detect skills source root. Run this script inside a project that contains ./skills/alpha, or pass --source explicitly (use ./skills/beta only when beta is intentional).",
...
MESSAGES_ZH = Messages(
    ...
    error_source_root_not_found="未能自动识别 skills 源目录：请在包含 ./skills/alpha 的项目目录中运行；beta 必须显式使用 --source，或直接传入 --source 指定其它源目录。",
