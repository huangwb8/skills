...
# 输入：[raw/ 或统一 products_dir 中的相对路径]
# 参数：[只列影响本单元科学结果的参数]
# 输出：<products_dir>/<流程>/AA.BB.CC. 名称/

source("00.Environment.R")
source(file.path("scripts", "lib", "checkpoint_helpers.R"))
...
code_files <- c(paste0(unit_stem, ".R"), unit_functions, file.path("scripts", "lib", "checkpoint_helpers.R"))
...
if (identical(decision, "hit")) {
  ...
  message("[PASS] checkpoint committed: ", file.path(getOption("bensz.products_dir", "products"), workflow, unit_stem))
}
