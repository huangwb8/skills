...
### Changed（变更）
- **Verifier 定位分层**：调用从 `markdown.references` 改为 `markdown.link-integrity`，只表达 Markdown 链接提取、站内锚点和 URL 可达性；补充格式无关的 `citation.truth-and-fit` 能力契约，明确引用真实性与适切性需要正文上下文、来源元数据和来源证据。Skill 版本升级至 `0.6.0`。
