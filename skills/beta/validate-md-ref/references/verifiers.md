...
```bash
bsk verifier run markdown.link-integrity --version 1.0.0 --input DOCUMENT.md \
  --events TASK_ROOT/log/events.ndjson --run-id RUN_ID
```

再运行 `scripts/validate_links.py`，传入同一个 `--events` 与 `--run-id`，以保留 `citation.truth-and-fit@1.0.0` 的结果。直接调用链接完整性 Verifier时不会自动读取 Skill 的 `config.yaml`；超时、白名单和黑名单必须通过 CLI 参数显式传入。任何 Verifier、Gate 或事件写入失败都必须终止本次检查，不得降级为仅脚本或手工检查。
